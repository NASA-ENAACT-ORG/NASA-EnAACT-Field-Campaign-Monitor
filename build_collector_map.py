...
# ── Load routes ────────────────────────────────────────────────────────────────
with open(BASE / "routes_data.json") as f:
...
try:
    v2_path = BASE / "V2_Preferred_Routes.xlsx"
    if v2_path.exists():
...
try:
    v1_path = BASE / "Preferred_Routes.xlsx"
    if v1_path.exists():
...
try:
    awb_path = BASE / "Collector_Schedule" / "Availability.xlsx"
    if awb_path.exists():
        import openpyxl
