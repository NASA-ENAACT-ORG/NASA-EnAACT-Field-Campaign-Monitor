...
2. Navigates (or creates) the correct borough → route subfolder hierarchy in Drive
3. Creates a walk folder named after the walk code
4. Uploads all files into typed subfolders
5. Calls `_run_drive_poll("upload")` which rebuilds `Walks_Log.txt` from Drive state and triggers a dashboard rebuild
...
| Start / Walk / End Time | Optional | Each can be an image upload or typed HH:MM:SS UTC |
| POM / POP / PAM | Optional | Individually toggled checkboxes; any file type, multiple files |
| GPX Track | Optional | .gpx, .kml, .kmz |
...
The `_run_drive_poll("upload")` call runs in a background thread after the upload response is sent. It can take 10–30 seconds. If it still doesn't appear:
- Confirm the walk folder was created in Drive (if not, the Drive write is failing entirely)
