 ...
 * modified since the last check. If it has, triggers a weather + dashboard
 * rebuild on Cloud Run. Self-scheduled assignments are preserved.
 *
 * SETUP (one-time):
 ...
 * it will treat the sheet as changed and trigger a rebuild, which is safe.
 */
 ...
 * against the value stored in ScriptProperties. Triggers /api/force-rebuild only
 * if the sheet has changed since the last successful check.
 */
function checkForecast() {
  ...
  if (currentMtime <= storedMtime) {
    Logger.log("No changes detected in forecast spreadsheet - skipping rebuild.");
    return;
  }

  Logger.log("Forecast spreadsheet updated - triggering rebuild on Cloud Run.");
  ...
  try {
    ...
    Logger.log("Rebuild HTTP " + code);

    if (code === 200) {
      // Persist new mtime only on success so a failed rebuild retries next cycle.
      props.setProperty("FORECAST_MTIME", currentMtime.toString());
      Logger.log("Forecast state updated (mtime=" + currentMtime + ").");
    } else {
      Logger.log("Rebuild failed - will retry tomorrow. Response: " + resp.getContentText());
    }
  } catch (err) {
