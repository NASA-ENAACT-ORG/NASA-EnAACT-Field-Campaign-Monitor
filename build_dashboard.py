...
<title>NASA EnAACT Field Campaign Monitor</title>
...
#header h1{font-size:14px;font-weight:700;white-space:nowrap;letter-spacing:-.3px}
...
.tab-btn{padding:4px 13px;background:transparent;border:1px solid transparent;border-radius:6px;color:var(--text2);cursor:pointer;font-size:12px;font-weight:500;transition:all .15s}
.tab-btn:hover{background:var(--bg3);color:var(--text)}
.tab-btn.active{background:var(--bg3);border-color:var(--border);color:var(--text)}
#filters{display:flex;align-items:center;gap:7px;margin-left:auto;flex-wrap:nowrap}
...
<div id="app">
  <div id="header">
    ...
    <div id="tabs">
      ...
      <button class="tab-btn" data-view="schedule-view">&#x1F4C5; Schedule</button>
...
function initMap(){
  ...
  for(const[code,geo]of Object.entries(ROUTES_GEO)){
    ...
    for(const line of geo.lines){
      const L2=L.polyline(line,{color:'#888',weight:4,opacity:.8}).addTo(map);
      L2.on('click',()=>openPanel(code));
...
function renderCollectorSelector(){
  ...
  el.innerHTML=COLLECTORS.map(cid=>{
    ...
    return`<div class="cc${cid===currentCollector?' active':''}" data-cid="${cid}">
      <div class="cn">${CNAMES[cid]}</div><div class="ci">${cid}</div>
      <div class="cw">${tot}</div><div class="cwl">filtered walks</div>
    </div>`;
  }).join('');
...
function bindEvents(){
  ...
  document.getElementById('sched-file').addEventListener('change',e=>{
    ...
    r.onload=ev=>{loadScheduleJSON(ev.target.result);e.target.value='';};
    r.readAsText(f);
  });
...
async function init(){
  ...
  try{
    const sr=await fetch('schedule_output.json').catch(()=>null);
    if(sr&&sr.ok){try{schedData=await sr.json();}catch(e){}}
  }catch(e){}
  try{
    ...
    updateMapStats();
    bindEvents();
    toast(src?'Log loaded from disk':'Using embedded sample data','success');
  }catch(err){
